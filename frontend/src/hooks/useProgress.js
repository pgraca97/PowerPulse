...
const COMPLETE_EXERCISE = gql`
  mutation CompleteExercise($exerciseId: ID!, $userId: ID!) {
    completeExercise(exerciseId: $exerciseId, userId: $userId) {
      exerciseType {
        id
        title
      }
...
export function useProgress() {
  ...
  const { data: progressData, loading: progressLoading, error: progressError, refetch: refetchProgress } = useQuery(GET_USER_PROGRESS, {
  ...
  const completeExercise = async (exerciseId) => {
    try {
      ...
      if (!userId) {
        throw new Error('User ID not found in user object. Available fields: ' + Object.keys(user).join(', '));
      }
